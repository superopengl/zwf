...
export const addDemplateToTask = handlerWrapper(async (req, res) => {
  ...
  await db.transaction(async m => {
    ...
    await emitTaskEvent(m, ZeventName.TaskAddedDoc, taskId, userId, taskDocs);
  });
...
export const assignTask = handlerWrapper(async (req, res) => {
  ...
  await db.transaction(async m => {
    ...
    await emitTaskEvent(m, ZeventName.TaskAssigned, id, userId, { assigneeId });
  });
...
const statusMapping = new Map([
  [`${TaskStatus.TODO}>${TaskStatus.IN_PROGRESS}`, ZeventName.TaskStatusToInProgress],
  [`${TaskStatus.TODO}>${TaskStatus.ACTION_REQUIRED}`, ZeventName.TaskStatusAwaitClient],
  [`${TaskStatus.TODO}>${TaskStatus.DONE}`, ZeventName.TaskStatusCompleted],
  [`${TaskStatus.TODO}>${TaskStatus.ARCHIVED}`, ZeventName.TaskStatusArchived],
  [`${TaskStatus.IN_PROGRESS}>${TaskStatus.TODO}`, ZeventName.TaskStatusMovedBackToDo],
  [`${TaskStatus.IN_PROGRESS}>${TaskStatus.ACTION_REQUIRED}`, ZeventName.TaskStatusAwaitClient],
  [`${TaskStatus.IN_PROGRESS}>${TaskStatus.DONE}`, ZeventName.TaskStatusCompleted],
  [`${TaskStatus.IN_PROGRESS}>${TaskStatus.ARCHIVED}`, ZeventName.TaskStatusArchived],
  [`${TaskStatus.ACTION_REQUIRED}>${TaskStatus.TODO}`, ZeventName.TaskStatusMovedBackToDo],
  [`${TaskStatus.ACTION_REQUIRED}>${TaskStatus.IN_PROGRESS}`, ZeventName.ClientSubmittedForm],
  [`${TaskStatus.ACTION_REQUIRED}>${TaskStatus.DONE}`, ZeventName.TaskStatusCompleted],
  [`${TaskStatus.ACTION_REQUIRED}>${TaskStatus.ARCHIVED}`, ZeventName.TaskStatusArchived],
  [`${TaskStatus.DONE}>${TaskStatus.TODO}`, ZeventName.TaskStatusMovedBackToDo],
  [`${TaskStatus.DONE}>${TaskStatus.IN_PROGRESS}`, ZeventName.TaskStatusToInProgress],
  [`${TaskStatus.DONE}>${TaskStatus.ACTION_REQUIRED}`, ZeventName.TaskStatusAwaitClient],
  [`${TaskStatus.DONE}>${TaskStatus.ARCHIVED}`, ZeventName.TaskStatusArchived],
  [`${TaskStatus.ARCHIVED}>${TaskStatus.TODO}`, ZeventName.TaskStatusMovedBackToDo],
  [`${TaskStatus.ARCHIVED}>${TaskStatus.IN_PROGRESS}`, ZeventName.TaskStatusToInProgress],
  [`${TaskStatus.ARCHIVED}>${TaskStatus.ACTION_REQUIRED}`, ZeventName.TaskStatusAwaitClient],
  [`${TaskStatus.ARCHIVED}>${TaskStatus.DONE}`, ZeventName.TaskStatusCompleted],
]);
...
export const requestClientAction = handlerWrapper(async (req, res) => {
  ...
  await db.transaction(async m => {
    ...
    await emitTaskEvent(m, ZeventName.RequestClientFillForm, id, userId);
    const message = comment?.trim();
    if (message) {
      await emitTaskEvent(m, ZeventName.TaskComment, id, userId, { message });
    }
...
export const getTaskTimeline = handlerWrapper(async (req, res) => {
  assertRole(req, ['admin', 'agent', 'client']);
  const { id } = req.params;
  let query: any = { taskId: id, type: Not(ZeventName.TaskComment) };
  const role = getRoleFromReq(req);
  switch (role) {
    case Role.Admin:
    case Role.Agent:
