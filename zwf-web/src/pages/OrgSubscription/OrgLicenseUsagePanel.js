            ...
            >
              ...
              {period.type === 'trial' ? <>
                ...
                <Paragraph>
                  ...
                  Upon the conclusion of the trial period, the system will automatically transition to a monthly plan at a rate of AUD $39.00 per account per 30 days (unless an alternative, agreed-upon price is in effect). The payment for the monthly plan will be processed on or after the last day of the period. For example, if a trial period ends on <Text underline>{moment(period.periodTo).format('MMM D YYYY')}</Text>, the first monthly plan will begin from <Text underline>{moment(period.periodTo).add(1, 'day').format('MMM D YYYY')}</Text>, and the initial payment will be processed around <Text underline>{moment(period.periodTo).add(30, 'days').add(-1, 'day').format('MMM D YYYY')}</Text>. Please note that the payment may be processed later than expected due to various reasons.
                </Paragraph>
