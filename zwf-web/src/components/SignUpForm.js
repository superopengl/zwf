...
const SignUpForm = (props) => {
  ...
  const handleSignIn = (values) => {
    ...
    signUp$(values).subscribe(() => {
      ...
      notify.success(
        ...
        <>Congratulations and thank you very much for signing up ZeeWorkflow. The invitation email has been sent out to <Text strong>{values.email}</Text>.</>
      );
    },
      err => { },
      () => setLoading(false));
  ...
  return (
    <ContainerStyled>
      ...
      <Link to="/signup/org"><Button type="primary" block>Create Organization</Button></Link>
    </ContainerStyled>
  );
}
